...
<body onload="document.getElementsByTagName('main')[0].style.opacity = '1'">
    <header>
        ...
        <a href="extensao.php" id="select_menu">Extensão de Cílios</a>
        <a href="manutencao.php">Manutenção de Extensão de Cílios</a>
        <a href="brow.php">Brow Lamination</a>
    </header>
    <main>
        <form action="../php/agendar.php" method="post" id="extensao" autocomplete="off">
            Dia:
            <input type="date" name="dia" id="dia" oninput="window.location.replace('extensao.php?dia='+this.value)" required>
            <?php
                if(isset($_GET['dia'])) {
                    $horarios_ocupados = consultaHorariosOcupados("extensao");
                    exibirHorarios("extensao", $horarios_ocupados, "Extensão de Cílios");
                }
